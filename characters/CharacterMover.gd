...
func sprint(sprinting):
	sprintPressed = sprinting
...
func _physics_process(delta):
	if frozen:
		return
	var cur_move_vector = move_vec
	if !ignore_rotation:
		cur_move_vector = cur_move_vector.rotated(Vector3.UP, body_to_move.rotation.y)
	velocity += move_accel * cur_move_vector - velocity * Vector3(drag, 0, drag) + gravity * Vector3.DOWN * delta
